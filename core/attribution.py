...
def multi_period_attribution(
    ...
    # Aggregate returns
    port_rets = pd.Series([df["rp"].sum() for df in single_period_results]) # sum weighted return?
    # Actually df["rp"] in brinson is asset return. Portfolio return is sum(wp * rp)
    ...
    def carino_k(R):
        return np.log(1 + R) / R if abs(R) > 1e-6 else 1.0

    k_total = (np.log(1 + total_Rp) - np.log(1 + total_Rb)) / (total_Rp - total_Rb) if abs(total_Rp - total_Rb) > 1e-6 else 1.0
    
    aggregated = pd.DataFrame(0.0, index=single_period_results[0].index, columns=["allocation", "selection", "interaction"])
